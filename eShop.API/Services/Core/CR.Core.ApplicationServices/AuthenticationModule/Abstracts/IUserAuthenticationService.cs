...
namespace CR.Core.ApplicationServices.AuthenticationModule.Abstracts
{
    ...
    public interface IUserAuthenticationService
    {
        ...
        /// <returns></returns>
        Task<Result<Users>> ValidateWebUser(string userName, string password);
        /// <summary>
        /// xác thực người dùng khi đăng nhập app ( có thể là customer hoặc admin)
        /// </summary>
        Task<Result<Users>> ValidateAppUser(string userName, string password);
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        Task<Result<Users>> FindUserAuthorizationById(int id);

        Task<Result> ForgotPasswordAsync(string email);
        Task<Result<object>> VerifyOtpForResetAsync(string email, string otpCode);
